...
在作战室消息框中输入以 `@AI` 开头的消息，AI助手会基于当前事件上下文（事件sumamry信息）提供专业建议：
...
AI助手具备强大的上下文感知能力，会自动获取：

- **当前事件详情**：包括事件描述、来源、严重程度等基本信息
- **处理历史**：事件的处理进度、已执行的操作和结果
- **Agent协作记录**：各个Agent的分析结果和建议
- **执行结果**：SOAR平台的执行反馈和威胁情报查询结果
...
#### 3. AI助手响应
AI助手会基于当前事件的完整上下文信息（包括事件详情、处理历史、Agent分析结果等）提供专业建议。
