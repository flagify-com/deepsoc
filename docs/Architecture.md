...
DeepSOC实现了完善的用户消息身份识别和显示机制：

#### 数据模型设计
- 用户表使用 `user_id` 字段（UUID格式）作为唯一标识
- 消息表通过 `user_id` 字段关联发送者
- 支持用户昵称(`nickname`)和用户名(`username`)两级显示

#### 消息显示逻辑
- **优先级1**: 消息中的 `user_nickname` 字段（优先显示用户昵称）
- **优先级2**: 消息中的 `user_username` 字段（备选显示用户名）
- **优先级3**: localStorage中的当前用户信息（当前用户消息）
- **优先级4**: 默认显示"其他用户"（无法识别的用户消息）

#### 实时消息传递
- 后端在消息广播时自动查询用户信息并附加到消息中
- 前端通过WebSocket接收包含完整用户信息的消息
- 支持多用户同时在线并正确显示各自的消息来源

详细的用户消息显示逻辑请参考：[User_Message_Display_Logic.md](User_Message_Display_Logic.md)
